...
interface IInputOptional extends IComponentBaseProps {
  id: string;
  filled: boolean;
  focused: boolean;
  required: boolean;
...
type IInputArguments = Optional<IInputOptional> & Optional<IInputRequired>;

export type IInputProps = Partial<Optional<IInputOptional>> &
  IInputRequired &
  IInputAttributes;

const defaults: IInputArguments = {
  selector: "Input",
  extendor: undefined,
  ref: undefined,
  className: undefined,
...
export function Input(args: IInputProps): JSX.Element {
  const componentContext = useComponentContext();
  const formControlContext = useContext(FormControlContext);
  const [props, others] = useProps(args, defaults);
  const { toClass, toStyle } = useCSS(props);

  const _disabled = () => props.disabled ?? componentContext?.disabled ?? formControlContext?.disabled;
