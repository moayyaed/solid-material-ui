...
import "./OutlinedInputStyle";

interface IOutlinedInputOptional extends IComponentBaseProps {
  id: string;
  filled: boolean;
  focused: boolean;
  required: boolean;
  ...
  labelWidth: number;
  notchedClass: string;
  notchedStyle: JSX.CSSProperties;
}

interface IOutlinedInputRequired {}

type IOutlinedInputAttributes = Omit<
  JSX.HTMLAttributes<HTMLInputElement>,
  "children" | "onChange" | "autoCapitalize" | "onFocus" | "onBlur"
>;

type IOutlinedInputArguments = Optional<IOutlinedInputOptional> &
  Optional<IOutlinedInputRequired>;

export type IOutlinedInputProps = Partial<Optional<IOutlinedInputOptional>> &
  IOutlinedInputRequired &
  IOutlinedInputAttributes;

const defaults: IOutlinedInputArguments = {
  ...
  notchedClass: undefined,
  notchedStyle: undefined,
};

export function OutlinedInput(args: IOutlinedInputProps): JSX.Element {
  ...
  const _notchedClass = toClass({
    section: "NotchedOutline",
    include: true,
    *class() {
      yield props.notchedClass;
    },
  });

  const _notchedStyle = toStyle({
    *style() {
      yield props.notchedStyle;
    },
  });
  ...
  return (
    <InputBase
      ...
      extendor="OutlinedInput"
      suffix={(args) => (
        <NotchedOutline
          disabled={_disabled()}
          class={_notchedClass()}
          style={_notchedStyle()}
          notched={args.notched}
